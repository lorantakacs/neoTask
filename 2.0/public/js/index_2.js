...
//sort last five votes
  var getLastFive = function(tArray){
    var voteTimeObj = [];
    for (m = 0; m < tArray.length; m++){
      if (tArray[m].votes > 0){
        voteTimeObj.push([tArray[m].voteTime, tArray[m].url]);
      };
    };
    return voteTimeObj;
  };
  ...
  voteTimeObjToSort = getLastFive(picArray);

  sortedVoteTimeObj = voteTimeObjToSort.sort(function(ob1, ob2){
    return ob2[0] - ob1[0];
  });

//display last five votes
  $('#last_five_vote').empty();

  var displayLastVotes = 0;

  if (sortedVoteTimeObj.length < 5){
    displayLastVotes = sortedVoteTimeObj.length;
  } else {
    displayLastVotes = 5;
  };

  for(n = 0; n < displayLastVotes; n++){
    $('#last_five_vote').append(`<img src="${sortedVoteTimeObj[n][1]}">`);
  };
